...
//Code Here
let evenNumbers = mixedNumbers.filter(cb => cb % 2 === 0)
...
//Code Here
let postTaxPrices = prices.map(cb => cb * 1.07);
...
//Code Here
let myStrongest = monstersInYourPocket.filter(cb => cb.CP > 200)
...
let orderTotals = orders.map(cb => cb.price * cb.tax + cb.price)
...
let bobsTotal = purchases.filter(cb =>
  cb.owner === "Bob"
).map(cb => cb.price
